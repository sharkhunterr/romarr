#!/usr/bin/env node
/**
 * Flexible release script for Romarr
 *
 * Features:
 * - Bump version using standard-version (package.json + pyproject.toml +
 *   src/romarr/__init__.py, all kept in lock-step via .versionrc.json)
 * - Push to GitLab and/or GitHub
 * - Create releases on GitLab and/or GitHub with content from
 *   GITHUB_RELEASES.md
 * - Deploy to Docker Hub (triggers GitLab CI)
 ...
 *
 * NOTE on repo layout: this script lives at the GIT ROOT. The romarr
 * project itself is nested one level down in ``romarr/`` — the CI and
 * the version updaters already account for that, releases run from the
 * git root.
 */
...
// Main release flow
async function main() {
  console.log('🚀 Romarr Release Script\n');
  ...
  if (!options.skipRelease) {
    ...
    const releaseNotesFile = '/tmp/romarr-release-notes.md';
    ...
    if (options.gitlab && gitlabUrl) {
      ...
      try {
        ...
        console.log('   The GitLab release is also created by GitLab CI (release:gitlab job).');
      }
    }
    ...
    if (options.github) {
      ...
      const githubRepo = process.env.GITHUB_REPO || 'sharkhunterr/romarr';
