...
from romarr.platform_packs.api import (
    config_router as platform_pack_config_router,
    packs_router,
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    if enable_bootstrap:
        ...
        # Built-in Platform Pack (T038). Gated by
        # ``platform_pack_config.builtin_enabled`` — operators can
        # opt out from Settings > Platforms if they want a
        # community-only setup.
        try:
            ...
            async with sm() as session:
                ...
                await session.commit()
                builtin_enabled = cfg.builtin_enabled
            if not builtin_enabled:
                bootstrap_log.info(
                    "lifespan.apply_builtin_pack",
                    extra={"applied": False, "skipped_reason": "config_disabled"},
                )
            else:
                async with sm() as session:
