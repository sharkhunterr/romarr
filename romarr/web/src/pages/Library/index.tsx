...
export function LibraryPage(): ReactElement {
  ...
  // Advanced-filter count: filters that live in the collapsible
  // panel below the always-visible quick row (search + platform
  // + sort + selection). Used to badge the "Filters ⌄" toggle so
  // the operator sees how many are active without expanding.
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6 md:px-6 md:py-8">
      <header className="mb-6 space-y-3">
        ...
        {/* Quick row — always visible: search + platform + sort
            direction. Everything else (tag / library / genre /
            region / year / monitored-only / reset) hides under
            the Filters ⌄ toggle.

            The selection-mode toggle stays here too because it's
            a mode, not a filter. */}
        <div className="flex flex-col gap-2 md:flex-row md:items-center">
          ...
          <button
            ...
            className={[
              "inline-flex shrink-0 items-center gap-1.5 rounded-md px-3 py-2 text-xs font-medium ring-1 ring-inset",
              "transition-colors",
              advancedFiltersCount > 0
        ...
        {/* Advanced filters — hidden by default, expanded via
            the Filters ⌄ toggle. Pre-expanded on first render
            iff at least one advanced filter is already set
            (initial state derived from advancedFiltersCount). */}
        {advancedOpen && (
