...
The **community-first release**. Platforms + Custom Formats no
longer ship as wheel-bundled seeds — they land from operator-
registered community URLs (with an offline JSON/ZIP fallback), get
merged across multiple sources via per-slug bindings, and are
surfaced through a unified Update Center in the header.

### Added

- **Update Center** — new central hub for every community-URL-driven
  resource (Custom Formats + Platform Packs today, extensible per
  ``resource_type``). Lives at Paramètres → Mises à jour and is
  embedded on both Custom Formats and Platforms pages so a source
  added anywhere shows up everywhere.
  - Aggregated header badge (green / amber / red) with a popover
    that lists every pending update from Romarr GitHub + every
    community source. One-click Apply from the popover.
  - Card layout with URL shortener (`raw.githubusercontent.com/...`
    → `github.com/owner/repo`), 4-state version pill (never
    synced / new / up-to-date / update available), colour-coded
    action icons (Check / Preview / Edit / Trust / Apply /
    Enable / Delete).
  - Editable name + URL per source (URL change resets version
    tracking + flips trust back to pending — a URL swap is
    semantically a new source).
  - Preview modal — dry-run manifest fetch, never touches the DB.
  - Local file import (JSON with `inline_items` or ZIP with
    `manifest.json` + item files) for air-gapped installs, 5 MiB
    cap.

- **Community-first platform bootstrap** (migration 0042) —
  removes the wheel-bundled builtin auto-apply. First boot
  preseeds a "Romarr Official Platforms" source pointing at
  `sharkhunterr/romarr-plateform-pack` and auto-fetches +
  auto-applies it in the background so the server starts serving
  immediately. Offline installs get a clear empty-state banner
  pointing at the URL / file paths above.

- **Per-source overrides for platforms** (migrations 0043, 0044) —
  when multiple sources declare the same slug the operator gets
  three modes per (source, slug):
  - `use` (default) — arrays merge across sources; scalars decided
    by rank.
  ...
  Plus a global `source_order` (drag-drop rank editor) to break
  scalar ties when no `prefer` fires. The materializer stores per-
  (source, slug) JSON snapshots in `platform_source_contribution`
  and rewrites the live `platform` row from the aggregate: scalars
  from the winner, arrays (aliases, formats, naming_tokens,
  newznab_ids) union-merged across every non-skipped source.

- **Custom Format Update Center** — CFs can now be imported in bulk
  from a URL manifest (`kind: "custom_format"`). Each CF row
  surfaces an origin badge (Built-in / Community · pack name /
  Manual) and a compact on/off toggle. Disabled CFs stay in the
  table but the search + importer pipelines skip them when
  aggregating scores (`preload_custom_formats` + importer context
  both filter `enabled=True`). Toggling doesn't flip
  `is_user_modified` — community sync keeps overwriting the seed
  body normally after the flip.

- **Community sync scheduler** — new `CommunitySync` task runner
  (seeded ON by default at `0 */6 * * *`) does a check-only sweep
  across every enabled + auto_check source, refreshing
  `last_seen_version` so the header badge lights up amber without
  operator action. Legacy `PackSourcesSync` runner now filters on
  `resource_type='platform_pack'` (defence against picking up CF
  rows as YAML).

- **Post-add edit modal for games** — new "Modifier" button on the
  game detail header lets the operator change monitoring +
  library binding after add (was previously only possible at add
  time). Backed by a new `useUpdateGame` hook (PATCH
  `/api/v3/game/{id}` accepting partial payloads).

- **Version-check endpoint + UI badge** — `/api/v3/system/version-check`
  hits GitHub releases with a 1 h in-process cache; the header
  badge shows the current version and turns amber when a newer
  release is available. Also mirrored on Grabarr.

### Changed

- **Platforms page** — the "Platform Packs" application history and
  the built-in toggle panel are gone (both obsolete with the
  community-first model). The catalogue grid becomes 1/2/3
  columns with proper visual hierarchy : name + up-to-3 colour-
  coded source-initial badges, short_name pill · slug ·
  manufacturer · year, aliases summary. Click still opens the
  detail modal (now with a "Sources qui contribuent" section
  showing the winner + all contributors).

- **Custom Formats page** — compact row layout with icon actions
  (toggle switch + chevron + Pencil + Trash), single-line header,
  conditions expand under the chevron. Origin badges next to the
  score chip.

- **Custom Format matching** — subtitle-separator guard rejects
  "The Legend of Zelda" fuzzy-matching "The Legend of Zelda -
  Skyward Sword" (Wii U game that isn't the NES original) or
  "Zelda: Ocarina of Time" via ` - ` / `: ` separator detection
  with leading-article strip. Prevents entire families of cross-
  franchise false positives.

- **Platform-mismatch guard** — pipeline hard-rejects releases
  whose file extension is declared native for another platform
  (`.wbfs` on NES = physically impossible). Uses the existing
  `PlatformFormatBounds` table as the extension → platform
  mapping. Complements the fuzzy-title matching layer above.

- **Manifest-driven `pack_version`** — operators can omit
  `pack_version` from platform-pack YAMLs and let `manifest.version`
  carry the single source of truth. The adapter injects it at
  ingest time. YAML-declared `pack_version` still wins for backward
  compat and per-file versioning.

### Fixed

- **`platform_source_contribution` backfill** — installs that
  applied a community pack BEFORE migration 0044 landed had a
  populated `platform` table but zero contributions (materializer
  ran with no data → empty UI badges). `_record_skipped` (called
  when the same pack version is re-applied) now walks parsed
  platforms and upserts a snapshot for every slug missing one,
  stamps `platform.pack_source_id` on rows that predate migration
  0043, and calls the materializer on the backfilled slugs. Heals
  on the next re-apply without operator action.

- **Friendly 503 when DB predates migration 0040** — the community
  endpoints used to 500 with a raw `sqlalchemy.OperationalError:
  no such column: pack_sources.resource_type` on stale DBs. Both
  GET routes now catch column-missing errors and return a 503
  with an actionable message pointing at
  `ROMARR_AUTO_MIGRATE=true romarr serve` /
  `alembic upgrade head`. Frontend renders it as an amber banner
  instead of "Internal Server Error".

- **CF adapter respects `is_user_modified`** — a community pack
  never overwrites an operator-edited CF; a warning surfaces
  instead so the operator can inspect and act.

- **New CF seed rejectors** — `aftermarket-bootleg-pirate`
  (-10000) and `homebrew-unlicensed` (-1000) added to the factory
  seed set so the "Legend of Zelda - Minish Cap Aftermarket
  Pirate" class of release is auto-rejected out of the box.

- **qBittorrent v5.x cookie name** (`QBT_SID_<port>`) with a
  subnet-bypass probe fallback for the reconciler.

- **BEP-53 file selection** wire-through (Grabarr
  `internal_file_path` → qBit `filePrio` background task) so
  bundle magnets (like MiNERVA / Myrient) don't start with 0/N
  files selected.

- **Radarr-style remote path mapping** on download clients
  (`remote_path` / `local_path` columns) — fixes the "completed
  file missing on disk" symptom when Romarr and qBit run in
  different mount namespaces.

### Migrations

- `0040` — Generalises `pack_sources` into the unified
  community-source table (`resource_type`, `last_seen_version`,
  `installed_version`, `auto_check`, `trust_status`).
- `0041` — `custom_format.enabled` (bool) + `source_id` FK for CF
  origin tracking.
- `0042` — Preseeds the official platform source; sets
  `platform_pack_config.builtin_enabled = false`.
- `0043` — `platform.pack_source_id` FK + `platform_source_binding`
  table (per-(source, slug) mode: skip / prefer / merge).
- `0044` — `platform_source_contribution` snapshot table +
  `platform_pack_config.source_order` JSON list.

### Frontend touchpoints

- New: Update Center page, CommunitySourcesPanel (embeddable),
  BindingsModal (3-way radio), SourceOrderPanel (drag-drop),
  PreviewModal, ImportLocalFileModal, EditCommunitySourceModal,
  UpdateCenterBadge (replaces the legacy VersionBadge),
  PlatformCard, EditGameModal.
- i18n: FR + EN across every new surface.
- 262 vitest tests, 279 backend tests (community + platform_packs
  + profiles + endpoint coverage), typecheck clean.
